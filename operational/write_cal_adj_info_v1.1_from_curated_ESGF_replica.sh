...
# Set up some paths
ESGF_DIR="/home/ucfaccb@ad.ucl.ac.uk/PMIP4-on-Estonia/curated_ESGF_replica"
THIS_DIR=`pwd`
echo $PWD
...
#PMIP4
pmip4_gcms="ACCESS-ESM1-5 AWI-ESM-1-1-LR CESM2 CESM2-FV2 CNRM-CM6-1 EC-Earth3-LR FGOALS-f3-L FGOALS-g3 GISS-E2-1-G HadGEM3-GC31-LL INM-CM4-8 IPSL-CM6A-LR MIROC-ES2L MPI-ESM1-2-LR MRI-ESM2-0 NESM3 NorESM1-F NorESM2-LM UofT-CCSM-4"
      ...
      do
        if [ $ncfile != "*.nc" ]; then #Only enter this loop if there are any monthly nc files
          ...
          if [ $NO_OVERWRITE == "TRUE" ] && [ -f $output_file ]; then 
            ...
            if [ $INFO_HEADER_WRITTEN == 0 ]; then
              echo  "activity,variable,time_freq,calendar_type,begageBP,endageBP,agestep,begyrCE,nsimyrs,interp_method,no_negatives,match_mean,tol,source_path,source_file,adjusted_path,adjusted_file"
              INFO_HEADER_WRITTEN=1  
            fi 
            #manipulate string
            no_nc=`echo ${input_file%.nc}`
            ...
            end_yr=`echo ${yr_str##*-} | cut -c-4`
            let length=$((10#$end_yr))-$((10#$start_yr))+1
            calendar=`ncdump -h $pt/$ncfile | grep time | grep calendar | cut -d\" -f2`
            if [ $variable == "pr" ] || [ $variable == "siconc" ]; then
              # both precipitation and sea ice concentration cannot go negative, so set that switch to TRUE 
              no_negatives="TRUE"
            else
